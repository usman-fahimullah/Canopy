...
export function CandidateComparisonSheet({
  ...
  const getHighestMatchScore = (matchScores: (number | null)[]): number => {
    const validScores = matchScores.filter((m): m is number => m !== null && m > 0);
    return validScores.length > 0 ? Math.max(...validScores) : 0;
  };

  const getHighestExperience = (experiences: (number | null)[]): number => {
    const validExperiences = experiences.filter((e): e is number => e !== null && e > 0);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="max-w-4xl">
        ...
        {error && (
          <div className="bg-background-error/5 rounded-lg border border-border-error p-4">
            <p className="mb-4 text-body text-foreground-error">Failed to load candidates</p>
        ...
        {!isLoading && !error && candidates.length > 0 && (
          <div className="max-h-[calc(100vh-200px)] space-y-6 overflow-y-auto">
            ...
            <div className="border-t border-border-muted pt-6">
              <h4 className="text-foreground-default mb-4 text-caption-strong font-semibold">
                Match Score
              </h4>
              <div
                className="grid gap-4"
                ...
                {candidates.map((candidate, idx) => {
                  ...
                  return (
                    <div
                      key={candidate.id}
                      className={
                        isHighest ? "rounded-lg p-4 ring-2 ring-[var(--primitive-green-400)]" : ""
                      }
                    >
                ...
                {candidates.map((candidate) => {
                  ...
                  return (
                    <div key={candidate.id} className="space-y-2">
                      <p className="text-foreground-default text-body font-semibold">
              ...
              >
                {candidates.map((candidate, idx) => {
                  ...
                  const allScores = candidates.map((c) =>
                    getAverageScore(candidates, candidates.indexOf(c))
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      {candidate.applications[0]?.scores.length === 0 && (
                        <p className="mt-2 text-caption text-foreground-muted">No scores yet</p>
                      )}
                    </div>
                  );
                })}
              ...
              >
                {candidates.map((candidate) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-end justify-between">
                        <span className="text-foreground-default text-heading-sm font-bold">
                      ...
                      {candidate.yearsExperience && (
                        <p className="mt-1 text-caption text-foreground-muted">years</p>
                      )}
                    </div>
                  );
                })}
                ...
                {candidates.map((candidate) => (
                  <div key={candidate.id} className="space-y-2">
                    {candidate.notes.length > 0 ? (
                      <>
                        <p className="text-foreground-default text-caption-strong">
                          {candidate.notes.length} note{candidate.notes.length !== 1 ? "s" : ""}
                        </p>
                        <p className="line-clamp-3 text-caption text-foreground-muted">
