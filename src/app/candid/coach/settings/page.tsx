...
const SETTING_SECTIONS = [
  ...
  },
  {
    id: "payments",
    label: "Payments",
    description: "Manage Stripe and payouts",
    icon: CurrencyDollar,
  },
  {
    id: "notifications",
...
function ProfileSection({
  ...
  saving: boolean;
  onSave: (fields: { name: string; bio: string; headline: string; hourlyRate: string }) => void;
}) {
  const [name, setName] = useState(profile.name || "");
  const [bio, setBio] = useState(profile.bio || "");
  const [headline, setHeadline] = useState(profile.coachProfile?.headline || "");
  const [hourlyRate, setHourlyRate] = useState(profile.coachProfile?.hourlyRate?.toString() || "");
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[var(--radius-xl)] border border-[var(--border-muted)] bg-[var(--background-default)] p-6">
        ...
        <div className="mb-6 flex items-center gap-5">
          <Avatar src={profile.avatar} name={profile.name} size="lg" color="yellow" />
        ...
        {isEditing ? (
          <div className="space-y-4 border-t border-[var(--border-muted)] pt-5">
            ...
            <div className="flex items-center gap-3 pt-2">
              <Button
                ...
                size="sm"
                loading={saving}
                onClick={() => onSave({ name, bio, headline, hourlyRate })}
              >
                Save Changes
              </Button>
          ...
          <>
            {/* Bio */}
            {profile.bio && (
              <div className="mb-5 border-t border-[var(--border-muted)] pt-5">
            ...
            {/* Specialties */}
            {profile.coachProfile?.specialties && profile.coachProfile.specialties.length > 0 && (
              <div className="mb-5 border-t border-[var(--border-muted)] pt-5">
            ...
            {profile.coachProfile?.hourlyRate != null && (
              <div className="border-t border-[var(--border-muted)] pt-5">
                ...
                <p className="text-foreground-default text-body-strong font-semibold">
                  ${profile.coachProfile.hourlyRate}
                  <span className="text-body-sm font-normal text-foreground-muted"> / hour</span>
                </p>
              </div>
            )}
          </>
...
function AvailabilitySection() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-foreground-default text-heading-sm font-medium">Availability</h2>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mb-4 text-body-sm text-foreground-muted">
              Configure your weekly availability and timezone settings. Clients will only be able to
              book during the hours you set.
            </p>
            <Button
              variant="secondary"
              size="sm"
              rightIcon={<ArrowSquareOut size={16} />}
              onClick={() => router.push("/candid/coach/schedule")}
            >
              Go to Schedule
            </Button>
          </div>
...
function PaymentsSection({ profile }: { profile: ProfileData }) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-foreground-default text-heading-sm font-medium">Payments</h2>
          ...
          <div className="min-w-0 flex-1">
            {stripeConnected ? (
              ...
              <>
                ...
                <p className="mb-4 text-body-sm text-foreground-muted">
                  Connect your Stripe account to receive payments from coaching sessions. Payouts
                  are processed weekly.
                </p>
                <Button variant="secondary" size="sm" leftIcon={<StripeLogo size={16} />} disabled>
                  Connect Stripe
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-[var(--radius-xl)] border border-[var(--border-muted)] bg-[var(--background-default)] p-5">
          <p className="mb-1 text-caption text-foreground-muted">Current Hourly Rate</p>
          <p className="text-foreground-default text-heading-sm font-semibold">
            {hourlyRate != null ? `$${hourlyRate}` : "Not set"}
          </p>
        </div>
        <div className="rounded-[var(--radius-xl)] border border-[var(--border-muted)] bg-[var(--background-default)] p-5">
          <p className="mb-1 text-caption text-foreground-muted">Total Earnings</p>
      ...
      <div className="rounded-[var(--radius-xl)] border border-[var(--border-muted)] bg-[var(--background-subtle)] p-5">
        <p className="text-body-sm text-foreground-muted">
          Payouts are processed weekly via Stripe. Funds typically arrive in your bank account
...
function NotificationsSection() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-5 rounded-[var(--radius-xl)] border border-[var(--border-muted)] bg-[var(--background-default)] p-6">
        <SwitchWithLabel
          label="New bookings"
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <SwitchWithLabel
              ...
              onCheckedChange={(checked) => setProfileVisible(checked as boolean)}
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5 rounded-[var(--radius-xl)] border border-[var(--border-muted)] bg-[var(--background-default)] p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="tertiary" size="sm" leftIcon={<Download size={16} />} disabled>
            Download
            <Badge variant="neutral" size="sm" className="ml-2">
              Coming soon
            </Badge>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="destructive" size="sm" leftIcon={<Trash size={16} />} disabled>
            Delete Account
          </Button>
        </div>
      </div>
...
export default function CoachSettingsPage() {
  ...
  const handleSaveProfile = useCallback(
    async (fields: { name: string; bio: string; headline: string; hourlyRate: string }) => {
      setSaving(true);
      try {
            ...
            prev
              ? {
                  ...
                  coachProfile: {
                    ...
                    headline: fields.headline || prev.coachProfile?.headline,
