    ...
    <div>
      ...
      <section className="px-8 py-6 lg:px-12">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {[
            { label: "Active Roles", value: data.activeRolesCount, icon: BriefcaseMetal, href: "/employer/roles" },
            { label: "Candidates", value: data.candidateCount, icon: Users, href: "/employer/candidates" },
            { label: "New Applications", value: data.newApplicationCount, icon: Table, href: "/employer/candidates" },
            { label: "Hires", value: data.hiredCount, icon: UsersFour, href: "/employer/analytics" },
          ].map((stat) => (
            <Link
              key={stat.label}
              href={stat.href}
            ...
            {data.recentRoles.map((job) => (
              ...
              >
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--primitive-blue-100)]">
                  <BriefcaseMetal size={20} weight="fill" className="text-[var(--primitive-blue-600)]" />
                </div>
          ...
          <div className="space-y-3">
            {data.recentApplications.map((app) => (
              ...
              >
                ...
                <Badge variant={statusBadgeVariant(app.stage)}>
                  {app.stage}
                </Badge>
              </div>
            ))}
      ...
      {totalInPipeline > 0 && (
        <section className="px-8 py-6 lg:px-12">
          <h2 className="text-heading-sm font-medium text-foreground-default mb-4">
            Hiring Pipeline
          </h2>
          <div className="rounded-[16px] border border-[var(--primitive-neutral-200)] bg-white p-6">
            <div className="flex items-end gap-1">
              {PIPELINE_STAGES.map((stage) => {
                ...
                const height = totalInPipeline > 0 ? Math.max((count / totalInPipeline) * 120, 8) : 8;
                return (
                  <div key={stage} className="flex-1 flex flex-col items-center gap-2">
                    <span className="text-caption-strong text-foreground-default">{count}</span>
                    <div
                      className="w-full rounded-t-lg bg-[var(--primitive-blue-200)] transition-all"
                      style={{ height: `${height}px` }}
                    />
                    <span className="text-caption text-foreground-muted text-center">
                      {stage}
                    </span>
                  </div>
                );
