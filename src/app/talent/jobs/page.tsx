    ...
    const fetchJobs = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setJobs(fetched);
          setSavedIds(
            new Set(fetched.filter((j) => j.isSaved).map((j) => j.id))
          );
        }
      } catch (err) {
            ...
            {filteredJobs.map((job) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-body font-medium text-[var(--foreground-default)] truncate">
                    {job.title}
                  </p>
                ...
                {/* Location */}
                {job.location && (
                  <div className="hidden sm:flex items-center gap-1 text-caption text-[var(--foreground-muted)]">
                    <MapPin size={14} />
