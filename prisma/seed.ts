...
async function main() {
  ...
  if (supabaseAdmin) {
    ...
    const seedEmails = [
      // Employers (10)
      "jordan@canopy.co", "alex@canopy.co", "sam@canopy.co",
      "morgan@aurora-climate.canopy.co",
      "dana@verdant-systems.canopy.co", "kevin@terrawatt.canopy.co",
      "rachel@evergreen-tech.canopy.co", "tomas@canopy.co",
      "nina@terrawatt.canopy.co", "derek@evergreen-tech.canopy.co",
      // Seekers (40)
      "maya.thompson@canopy.co", "ryan.oconnor@canopy.co",
      "priya.sharma@canopy.co", "carlos.mendez@canopy.co",
      "elena.volkov@canopy.co",
      "aisha.johnson@canopy.co", "ben.nakamura@canopy.co",
      "chloe.dupont@canopy.co", "david.okafor@canopy.co",
      "fatima.hassan@canopy.co", "gabriel.santos@canopy.co",
      "hannah.kim@canopy.co", "ian.macleod@canopy.co",
      "jasmine.patel@canopy.co", "kai.anderson@canopy.co",
      "lucia.martinez@canopy.co", "marcus.williams@canopy.co",
      "nadia.petrova@canopy.co", "omar.farah@canopy.co",
      "penny.chen@canopy.co", "quinn.taylor@canopy.co",
      "rosa.gutierrez@canopy.co", "sanjay.reddy@canopy.co",
      "tara.obrien@canopy.co", "victor.chang@canopy.co",
      "wendy.adeyemi@canopy.co", "xavier.moreau@canopy.co",
      "yuki.tanaka@canopy.co", "zara.khan@canopy.co",
      "amber.whitfield@canopy.co", "brian.hernandez@canopy.co",
      "camille.nguyen@canopy.co", "dante.jackson@canopy.co",
      "eva.lindgren@canopy.co", "finn.oleary@canopy.co",
      "grace.wu@canopy.co", "hassan.ali@canopy.co",
      "iris.kowalski@canopy.co", "james.blackwood@canopy.co",
  ...
  const morganAuthId = await ensureAuthUser("morgan@aurora-climate.canopy.co", "Morgan Walsh", "seed_aurora_admin");
  const mayaAuthId = await ensureAuthUser("maya.thompson@canopy.co", "Maya Thompson", "seed_seeker_001");
  const ryanAuthId = await ensureAuthUser("ryan.oconnor@canopy.co", "Ryan O'Connor", "seed_seeker_002");
  const priyaAuthId = await ensureAuthUser("priya.sharma@canopy.co", "Priya Sharma", "seed_seeker_003");
  const carlosAuthId = await ensureAuthUser("carlos.mendez@canopy.co", "Carlos Mendez", "seed_seeker_004");
  const elenaAuthId = await ensureAuthUser("elena.volkov@canopy.co", "Elena Volkov", "seed_seeker_005");

  // New employer auth users
  const danaAuthId = await ensureAuthUser("dana@verdant-systems.canopy.co", "Dana Flores", "seed_employer_005");
  const kevinAuthId = await ensureAuthUser("kevin@terrawatt.canopy.co", "Kevin Park", "seed_employer_006");
  const rachelAuthId = await ensureAuthUser("rachel@evergreen-tech.canopy.co", "Rachel Adams", "seed_employer_007");
  const tomasAuthId = await ensureAuthUser("tomas@canopy.co", "Tomas Reyes", "seed_employer_008");
  const ninaAuthId = await ensureAuthUser("nina@terrawatt.canopy.co", "Nina Johansson", "seed_employer_009");
  const derekAuthId = await ensureAuthUser("derek@evergreen-tech.canopy.co", "Derek Osei", "seed_employer_010");

  // New seeker auth users (35 more)
  const aishaAuthId = await ensureAuthUser("aisha.johnson@canopy.co", "Aisha Johnson", "seed_seeker_006");
  const benAuthId = await ensureAuthUser("ben.nakamura@canopy.co", "Ben Nakamura", "seed_seeker_007");
  const chloeAuthId = await ensureAuthUser("chloe.dupont@canopy.co", "Chloe Dupont", "seed_seeker_008");
  const davidAuthId = await ensureAuthUser("david.okafor@canopy.co", "David Okafor", "seed_seeker_009");
  const fatimaAuthId = await ensureAuthUser("fatima.hassan@canopy.co", "Fatima Hassan", "seed_seeker_010");
  const gabrielAuthId = await ensureAuthUser("gabriel.santos@canopy.co", "Gabriel Santos", "seed_seeker_011");
  const hannahAuthId = await ensureAuthUser("hannah.kim@canopy.co", "Hannah Kim", "seed_seeker_012");
  const ianAuthId = await ensureAuthUser("ian.macleod@canopy.co", "Ian MacLeod", "seed_seeker_013");
  const jasmineAuthId = await ensureAuthUser("jasmine.patel@canopy.co", "Jasmine Patel", "seed_seeker_014");
  const kaiAuthId = await ensureAuthUser("kai.anderson@canopy.co", "Kai Anderson", "seed_seeker_015");
  const luciaAuthId = await ensureAuthUser("lucia.martinez@canopy.co", "Lucia Martinez", "seed_seeker_016");
  const marcusAuthId = await ensureAuthUser("marcus.williams@canopy.co", "Marcus Williams", "seed_seeker_017");
  const nadiaAuthId = await ensureAuthUser("nadia.petrova@canopy.co", "Nadia Petrova", "seed_seeker_018");
  ...
  const quinnAuthId = await ensureAuthUser("quinn.taylor@canopy.co", "Quinn Taylor", "seed_seeker_021");
  const rosaAuthId = await ensureAuthUser("rosa.gutierrez@canopy.co", "Rosa Gutierrez", "seed_seeker_022");
  const sanjayAuthId = await ensureAuthUser("sanjay.reddy@canopy.co", "Sanjay Reddy", "seed_seeker_023");
  const taraAuthId = await ensureAuthUser("tara.obrien@canopy.co", "Tara O'Brien", "seed_seeker_024");
  const victorAuthId = await ensureAuthUser("victor.chang@canopy.co", "Victor Chang", "seed_seeker_025");
  const wendyAuthId = await ensureAuthUser("wendy.adeyemi@canopy.co", "Wendy Adeyemi", "seed_seeker_026");
  const xavierAuthId = await ensureAuthUser("xavier.moreau@canopy.co", "Xavier Moreau", "seed_seeker_027");
  const yukiAuthId = await ensureAuthUser("yuki.tanaka@canopy.co", "Yuki Tanaka", "seed_seeker_028");
  const zaraAuthId = await ensureAuthUser("zara.khan@canopy.co", "Zara Khan", "seed_seeker_029");
  const amberAuthId = await ensureAuthUser("amber.whitfield@canopy.co", "Amber Whitfield", "seed_seeker_030");
  const brianAuthId = await ensureAuthUser("brian.hernandez@canopy.co", "Brian Hernandez", "seed_seeker_031");
  const camilleAuthId = await ensureAuthUser("camille.nguyen@canopy.co", "Camille Nguyen", "seed_seeker_032");
  const danteAuthId = await ensureAuthUser("dante.jackson@canopy.co", "Dante Jackson", "seed_seeker_033");
  const evaAuthId = await ensureAuthUser("eva.lindgren@canopy.co", "Eva Lindgren", "seed_seeker_034");
  const finnAuthId = await ensureAuthUser("finn.oleary@canopy.co", "Finn O'Leary", "seed_seeker_035");
  const graceAuthId = await ensureAuthUser("grace.wu@canopy.co", "Grace Wu", "seed_seeker_036");
  const hassanAuthId = await ensureAuthUser("hassan.ali@canopy.co", "Hassan Ali", "seed_seeker_037");
  const irisAuthId = await ensureAuthUser("iris.kowalski@canopy.co", "Iris Kowalski", "seed_seeker_038");
  const jamesAuthId = await ensureAuthUser("james.blackwood@canopy.co", "James Blackwood", "seed_seeker_039");
  ...
  const verdantSystems = await prisma.organization.create({
    data: {
      ...
      logo: null,
      description: "Circular economy platform helping manufacturers eliminate waste through smart materials tracking and lifecycle optimization.",
  ...
  const terraWatt = await prisma.organization.create({
    data: {
      ...
      slug: "terrawatt",
      logo: null,
      description: "Grid-scale energy storage company deploying next-generation battery systems to accelerate the renewable energy transition.",
  ...
  const evergreenTech = await prisma.organization.create({
    data: {
      ...
      slug: "evergreen-tech",
      logo: null,
      description: "Conservation technology company using AI and remote sensing to protect biodiversity and monitor ecosystem health at scale.",
  ...
  const danaMember = await prisma.organizationMember.create({
    data: { accountId: danaAccount.id, organizationId: verdantSystems.id, role: OrgMemberRole.OWNER, title: "Founder & CEO" },
  });

  // TerraWatt members
  const kevinMember = await prisma.organizationMember.create({
    data: { accountId: kevinAccount.id, organizationId: terraWatt.id, role: OrgMemberRole.OWNER, title: "CTO" },
  });
  const ninaMember = await prisma.organizationMember.create({
    data: { accountId: ninaAccount.id, organizationId: terraWatt.id, role: OrgMemberRole.RECRUITER, title: "Talent Acquisition Lead" },
  });

  // Evergreen Tech members
  const rachelMember = await prisma.organizationMember.create({
    data: { accountId: rachelAccount.id, organizationId: evergreenTech.id, role: OrgMemberRole.OWNER, title: "CEO" },
  });
  const derekMember = await prisma.organizationMember.create({
    data: { accountId: derekAccount.id, organizationId: evergreenTech.id, role: OrgMemberRole.ADMIN, title: "VP Engineering" },
  });

  // Extra member for Solaris
  const tomasMember = await prisma.organizationMember.create({
    data: { accountId: tomasAccount.id, organizationId: solarisEnergy.id, role: OrgMemberRole.ADMIN, title: "VP Operations" },
  });
  ...
  const aishaSeeker = await prisma.seekerProfile.create({
    data: { accountId: aishaAccount.id, headline: "Environmental Engineer | Water Quality Specialist", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Water Treatment Design", "Environmental Compliance", "AutoCAD", "GIS"], greenSkills: ["Stormwater Management", "Wetland Restoration"], certifications: ["PE License", "LEED AP"], yearsExperience: 6, aiSummary: "Solid environmental engineering background with specialization in water systems. PE licensed with strong compliance track record.", targetSectors: ["water", "infrastructure"] },
  });
  const benSeeker = await prisma.seekerProfile.create({
    data: { accountId: benAccount.id, headline: "UX Designer | Climate Tech Focused", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "exploring-options"], skills: ["Figma", "User Research", "Prototyping", "Design Systems"], greenSkills: ["Sustainability Dashboard Design", "Behavior Change UX"], certifications: [], yearsExperience: 5, aiSummary: "Talented UX designer looking to apply human-centered design skills to climate and sustainability products.", targetSectors: ["climate-tech"] },
  });
  const chloeSeeker = await prisma.seekerProfile.create({
    data: { accountId: chloeAccount.id, headline: "Marine Biologist | Coastal Restoration", careerStage: CareerStage.SENIOR, motivations: ["finding-job", "networking"], skills: ["Marine Ecology", "Field Research", "Grant Writing", "R"], greenSkills: ["Coral Restoration", "Marine Protected Areas", "Blue Carbon"], certifications: ["PADI Divemaster", "Wilderness First Responder"], yearsExperience: 10, aiSummary: "Experienced marine scientist with decade of coastal restoration fieldwork. Strong publication record and grant writing skills.", targetSectors: ["conservation", "research"] },
  });
  const davidSeeker = await prisma.seekerProfile.create({
    data: { accountId: davidAccount.id, headline: "Mechanical Engineer | EV Battery Systems", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Battery Design", "Thermal Management", "MATLAB", "SolidWorks"], greenSkills: ["EV Battery Pack Design", "Energy Storage Systems"], certifications: ["FE Exam Passed"], yearsExperience: 5, aiSummary: "Strong mechanical engineer pivoting from automotive to clean energy storage. Deep thermal management expertise.", targetSectors: ["clean-energy", "transportation"] },
  });
  const fatimaSeeker = await prisma.seekerProfile.create({
    data: { accountId: fatimaAccount.id, headline: "Sustainability Consultant | B Corp Specialist", careerStage: CareerStage.SENIOR, motivations: ["exploring-options", "networking"], skills: ["Sustainability Reporting", "Stakeholder Engagement", "Life Cycle Assessment", "ISO 14001"], greenSkills: ["B Corp Assessment", "Science Based Targets", "CDP Disclosure"], certifications: ["B Corp Consultant", "ISO 14001 Lead Auditor"], yearsExperience: 9, aiSummary: "Expert sustainability consultant who has guided 20+ companies through B Corp certification. Excellent at translating sustainability into business value.", targetSectors: ["corporate-sustainability", "consulting"], isMentor: true, mentorBio: "I love helping people navigate the sustainability consulting landscape. Ask me anything about certifications and career paths.", mentorTopics: ["B Corp Certification", "Sustainability Consulting Careers", "ESG Frameworks"], mentorSpecialty: "Sustainability Consulting", mentorBadge: "quick_responder", mentorRating: 4.9, mentorReviewCount: 5 },
  });
  const gabrielSeeker = await prisma.seekerProfile.create({
    data: { accountId: gabrielAccount.id, headline: "Solar Sales Manager | Community Solar Champion", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Solar Sales", "CRM Management", "Community Outreach", "Team Leadership"], greenSkills: ["Community Solar Programs", "Net Metering", "Solar Financing"], certifications: ["NABCEP PV Associate"], yearsExperience: 6, aiSummary: "Proven solar sales leader who has closed over $10M in community solar contracts. Strong community engagement skills.", targetSectors: ["clean-energy"] },
  });
  const hannahSeeker = await prisma.seekerProfile.create({
    data: { accountId: hannahAccount.id, headline: "Climate Policy Researcher | Just Transition Focus", careerStage: CareerStage.ENTRY_LEVEL, motivations: ["finding-job", "learning-skills"], skills: ["Policy Analysis", "Qualitative Research", "Technical Writing", "Spanish"], greenSkills: ["Environmental Justice", "Just Transition Frameworks"], certifications: [], yearsExperience: 2, aiSummary: "Recent MPP graduate with strong research skills and passion for equitable climate policy. Published thesis on just transition.", targetSectors: ["policy", "nonprofit"] },
  });
  const ianSeeker = await prisma.seekerProfile.create({
    data: { accountId: ianAccount.id, headline: "Circular Economy Analyst | Zero Waste Advocate", careerStage: CareerStage.CAREER_CHANGER, motivations: ["finding-job", "exploring-options", "learning-skills"], skills: ["Supply Chain Analysis", "Waste Auditing", "Excel", "Process Improvement"], greenSkills: ["Circular Economy Principles", "Waste Stream Analysis", "Material Recovery"], certifications: ["TRUE Zero Waste Advisor"], yearsExperience: 3, aiSummary: "Former supply chain analyst transitioning into circular economy work. Practical experience in waste auditing and material flow analysis.", targetSectors: ["circular-economy", "waste-management"] },
  });
  const jasmineSeeker = await prisma.seekerProfile.create({
    data: { accountId: jasmineAccount.id, headline: "Environmental Educator | Curriculum Designer", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Curriculum Development", "Public Speaking", "Program Management", "Grant Writing"], greenSkills: ["Climate Literacy", "Outdoor Education", "Youth Engagement"], certifications: ["Certified Environmental Educator"], yearsExperience: 7, aiSummary: "Passionate environmental educator with experience developing K-12 sustainability curricula reaching 10,000+ students.", targetSectors: ["education", "nonprofit"], isMentor: true, mentorBio: "Helping aspiring environmental educators find their path. I can advise on certifications, curriculum design, and breaking into the field.", mentorTopics: ["Environmental Education Careers", "Curriculum Design", "Nonprofit Job Search"], mentorSpecialty: "Environmental Education Careers", mentorRating: 4.6, mentorReviewCount: 4 },
  });
  const kaiSeeker = await prisma.seekerProfile.create({
    data: { accountId: kaiAccount.id, headline: "Climate Data Journalist | Storytelling with Impact", careerStage: CareerStage.MID_CAREER, motivations: ["exploring-options", "networking"], skills: ["Data Journalism", "Python", "D3.js", "Investigative Reporting"], greenSkills: ["Climate Data Visualization", "Environmental Reporting"], certifications: [], yearsExperience: 5, aiSummary: "Data journalist specializing in climate stories. Published in major outlets with strong Python and visualization skills.", targetSectors: ["media", "communications"] },
  });
  const luciaSeeker = await prisma.seekerProfile.create({
    data: { accountId: luciaAccount.id, headline: "Solar Installation Apprentice | Eager Learner", careerStage: CareerStage.STUDENT, motivations: ["finding-job", "learning-skills", "networking"], skills: ["Electrical Basics", "Blueprint Reading", "Hand Tools"], greenSkills: ["Solar PV Basics"], certifications: ["OSHA 10"], yearsExperience: 0, aiSummary: "Enthusiastic trade school student in solar installation program. Strong work ethic and eager to learn.", targetSectors: ["clean-energy"] },
  });
  const marcusSeeker = await prisma.seekerProfile.create({
    data: { accountId: marcusAccount.id, headline: "Climate Lobbyist | Legislative Strategy", careerStage: CareerStage.SENIOR, motivations: ["finding-job", "career-advancement"], skills: ["Legislative Advocacy", "Coalition Building", "Public Policy", "Government Relations"], greenSkills: ["Clean Energy Policy", "Climate Legislation", "Carbon Pricing"], certifications: [], yearsExperience: 12, aiSummary: "Seasoned government relations professional with track record of advancing clean energy legislation at state and federal levels.", targetSectors: ["policy", "government"], isMentor: true, mentorBio: "I can help you understand how climate policy careers work, from think tanks to Capitol Hill. Happy to share my network.", mentorTopics: ["Climate Policy Careers", "Government Relations", "Legislative Advocacy"], mentorSpecialty: "Climate Policy Careers", mentorRating: 4.9, mentorReviewCount: 6 },
  });
  const nadiaSeeker = await prisma.seekerProfile.create({
    data: { accountId: nadiaAccount.id, headline: "Green Building Architect | Passive House Designer", careerStage: CareerStage.SENIOR, motivations: ["finding-job", "networking"], skills: ["Revit", "Passive House Design", "Energy Modeling", "Project Management"], greenSkills: ["Net Zero Design", "Passive House Standard", "LEED Design"], certifications: ["CPHD (Certified Passive House Designer)", "LEED AP BD+C", "AIA"], yearsExperience: 11, aiSummary: "Award-winning architect specializing in passive house and net-zero buildings. Designed 15+ certified projects.", targetSectors: ["construction", "real-estate"] },
  });
  const omarSeeker = await prisma.seekerProfile.create({
    data: { accountId: omarAccount.id, headline: "Sustainable Agriculture Researcher | Soil Science", careerStage: CareerStage.ENTRY_LEVEL, motivations: ["finding-job", "learning-skills"], skills: ["Soil Analysis", "Lab Research", "Data Collection", "Scientific Writing"], greenSkills: ["Soil Carbon Sequestration", "Regenerative Practices"], certifications: [], yearsExperience: 1, aiSummary: "Recent agriculture science graduate with hands-on soil research experience. Strong lab skills and passion for regenerative farming.", targetSectors: ["agriculture", "research"] },
  });
  const pennySeeker = await prisma.seekerProfile.create({
    data: { accountId: pennyAccount.id, headline: "Product Manager | CleanTech SaaS", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Product Management", "Agile/Scrum", "SQL", "User Research", "Roadmap Planning"], greenSkills: ["Carbon Accounting Products", "ESG Software"], certifications: ["Certified Scrum Product Owner"], yearsExperience: 6, aiSummary: "Product manager with SaaS background now focused on climate tech products. Strong analytical skills and user empathy.", targetSectors: ["climate-tech"] },
  });
  const quinnSeeker = await prisma.seekerProfile.create({
    data: { accountId: quinnAccount.id, headline: "Ecotourism Guide | Conservation Storyteller", careerStage: CareerStage.CAREER_CHANGER, motivations: ["finding-job", "exploring-options"], skills: ["Nature Interpretation", "Group Facilitation", "Photography", "Wildlife ID"], greenSkills: ["Ecotourism Best Practices", "Leave No Trace"], certifications: ["Certified Interpretive Guide", "Wilderness First Aid"], yearsExperience: 2, aiSummary: "Former hospitality professional transitioning to ecotourism and conservation. Excellent communication and outdoor skills.", targetSectors: ["tourism", "conservation"] },
  });
  const rosaSeeker = await prisma.seekerProfile.create({
    data: { accountId: rosaAccount.id, headline: "Wind Turbine Technician | O&M Specialist", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Wind Turbine Maintenance", "Electrical Troubleshooting", "Safety Compliance", "Hydraulics"], greenSkills: ["Wind Energy O&M", "Predictive Maintenance"], certifications: ["GWO Basic Safety Training", "OSHA 30"], yearsExperience: 5, aiSummary: "Skilled wind technician with excellent safety record. Experienced with multiple turbine platforms and predictive maintenance systems.", targetSectors: ["clean-energy"] },
  });
  const sanjaySeeker = await prisma.seekerProfile.create({
    data: { accountId: sanjayAccount.id, headline: "Backend Engineer | Energy Grid Software", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Go", "Python", "Kubernetes", "PostgreSQL", "gRPC"], greenSkills: ["Grid Management Software", "Energy APIs"], certifications: ["AWS Solutions Architect"], yearsExperience: 7, aiSummary: "Strong backend engineer building distributed systems for energy grid management. Deep expertise in Go and infrastructure.", targetSectors: ["climate-tech", "clean-energy"] },
  });
  const taraSeeker = await prisma.seekerProfile.create({
    data: { accountId: taraAccount.id, headline: "Returning Professional | Environmental Compliance", careerStage: CareerStage.RETURNING, motivations: ["finding-job", "learning-skills", "networking"], skills: ["Environmental Compliance", "Permitting", "Regulatory Analysis", "Report Writing"], greenSkills: ["EPA Regulations", "Air Quality Monitoring"], certifications: ["CHMM"], yearsExperience: 8, aiSummary: "Experienced environmental compliance professional returning after a career break. Strong regulatory knowledge and permitting background.", targetSectors: ["corporate-sustainability", "consulting"] },
  });
  const victorSeeker = await prisma.seekerProfile.create({
    data: { accountId: victorAccount.id, headline: "Urban Farmer | Community Food Systems", careerStage: CareerStage.ENTRY_LEVEL, motivations: ["finding-job", "learning-skills"], skills: ["Urban Agriculture", "Composting", "Community Engagement", "Farm Management"], greenSkills: ["Urban Food Production", "Composting Systems", "Food Justice"], certifications: ["Organic Grower Certificate"], yearsExperience: 2, aiSummary: "Passionate urban farmer building community food systems. Hands-on experience managing a 2-acre urban farm serving 200+ families.", targetSectors: ["agriculture", "community-development"] },
  });
  const wendySeeker = await prisma.seekerProfile.create({
    data: { accountId: wendyAccount.id, headline: "ESG Program Manager | Supply Chain Sustainability", careerStage: CareerStage.SENIOR, motivations: ["finding-job", "career-advancement"], skills: ["Program Management", "Supply Chain Analysis", "Vendor Auditing", "Sustainability Reporting"], greenSkills: ["Scope 3 Emissions", "Supplier ESG Assessment", "Science Based Targets"], certifications: ["PMP", "GRI Certified"], yearsExperience: 9, aiSummary: "Senior ESG professional with deep supply chain sustainability experience. Led Scope 3 reduction programs for Fortune 500.", targetSectors: ["corporate-sustainability", "consulting"], isMentor: true, mentorBio: "I mentor people breaking into corporate sustainability, especially supply chain ESG roles. Happy to review resumes and do mock interviews.", mentorTopics: ["Corporate Sustainability Careers", "Supply Chain ESG", "Resume Review"], mentorSpecialty: "Corporate Sustainability Careers", mentorBadge: "featured", mentorRating: 5, mentorReviewCount: 12 },
  });
  const xavierSeeker = await prisma.seekerProfile.create({
    data: { accountId: xavierAccount.id, headline: "EV Charging Infrastructure Manager", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "exploring-options"], skills: ["Project Management", "Electrical Systems", "Permitting", "Vendor Management"], greenSkills: ["EV Charging Networks", "Grid Integration", "NEVI Program"], certifications: ["PMP", "EVSE Installer Certified"], yearsExperience: 5, aiSummary: "Infrastructure manager specializing in EV charging network deployment. Experience with both Level 2 and DC fast charger installations.", targetSectors: ["transportation", "clean-energy"] },
  });
  const yukiSeeker = await prisma.seekerProfile.create({
    data: { accountId: yukiAccount.id, headline: "Ocean Conservation Researcher | Coral Ecology", careerStage: CareerStage.ENTRY_LEVEL, motivations: ["finding-job", "learning-skills"], skills: ["Marine Research", "Scuba Diving", "GIS Mapping", "Python"], greenSkills: ["Coral Reef Monitoring", "Marine Conservation"], certifications: ["AAUS Scientific Diver"], yearsExperience: 1, aiSummary: "Recent marine biology graduate with fieldwork experience in coral reef ecosystems. Strong GIS and data analysis skills.", targetSectors: ["conservation", "research"] },
  });
  const zaraSeeker = await prisma.seekerProfile.create({
    data: { accountId: zaraAccount.id, headline: "Climate Finance Associate | Green Bonds", careerStage: CareerStage.ENTRY_LEVEL, motivations: ["finding-job", "career-advancement"], skills: ["Financial Modeling", "Bloomberg Terminal", "Risk Analysis", "Excel"], greenSkills: ["Green Bond Frameworks", "Climate Risk Assessment"], certifications: ["CFA Level I"], yearsExperience: 2, aiSummary: "Early-career finance professional with green bond experience. Strong quantitative skills and growing ESG knowledge.", targetSectors: ["finance", "climate-tech"] },
  });
  const amberSeeker = await prisma.seekerProfile.create({
    data: { accountId: amberAccount.id, headline: "Sustainability Communications Manager", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["PR Strategy", "Content Marketing", "Social Media", "Crisis Communications"], greenSkills: ["Greenwashing Prevention", "Sustainability Storytelling"], certifications: ["APR"], yearsExperience: 6, aiSummary: "Communications professional specializing in authentic sustainability messaging. Expert at avoiding greenwashing while driving brand value.", targetSectors: ["media", "corporate-sustainability"] },
  });
  const brianSeeker = await prisma.seekerProfile.create({
    data: { accountId: brianAccount.id, headline: "HVAC Technician | Building Decarbonization", careerStage: CareerStage.CAREER_CHANGER, motivations: ["finding-job", "learning-skills"], skills: ["HVAC Systems", "Heat Pump Installation", "Building Energy Audits", "Refrigerant Management"], greenSkills: ["Heat Pump Conversions", "Building Electrification"], certifications: ["EPA 608 Universal", "NATE Certified"], yearsExperience: 8, aiSummary: "Experienced HVAC tech transitioning to building decarbonization. Expert in heat pump installations and building electrification.", targetSectors: ["construction", "clean-energy"], isMentor: true, mentorBio: "I can help tradespeople understand the building decarbonization opportunity. Heat pumps are the future and we need skilled workers.", mentorTopics: ["Heat Pump Careers", "Building Trades to Clean Energy", "HVAC Certifications"], mentorSpecialty: "Heat Pump Careers", mentorBadge: "quick_responder", mentorRating: 4.9, mentorReviewCount: 4 },
  });
  const camilleSeeker = await prisma.seekerProfile.create({
    data: { accountId: camilleAccount.id, headline: "Climate Adaptation Planner | Urban Resilience", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "networking"], skills: ["Urban Planning", "Climate Modeling", "Community Engagement", "GIS"], greenSkills: ["Climate Adaptation Planning", "Flood Risk Assessment", "Heat Island Mitigation"], certifications: ["AICP Certified Planner"], yearsExperience: 6, aiSummary: "Urban planner focused on climate adaptation and resilience. Experience developing city-wide climate action plans.", targetSectors: ["urban-planning", "policy"] },
  });
  const danteSeeker = await prisma.seekerProfile.create({
    data: { accountId: danteAccount.id, headline: "Environmental Justice Organizer | Community Advocate", careerStage: CareerStage.ENTRY_LEVEL, motivations: ["finding-job", "networking", "learning-skills"], skills: ["Community Organizing", "Public Speaking", "Event Planning", "Grant Writing"], greenSkills: ["Environmental Justice", "Community Advocacy", "Frontline Communities"], certifications: [], yearsExperience: 2, aiSummary: "Grassroots organizer with strong community ties and passion for environmental justice. Skilled public speaker and event organizer.", targetSectors: ["policy", "nonprofit"] },
  });
  const evaSeeker = await prisma.seekerProfile.create({
    data: { accountId: evaAccount.id, headline: "Forest Ecologist | Carbon Offset Verification", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "career-advancement"], skills: ["Forest Inventory", "Remote Sensing", "Carbon Modeling", "R/Python"], greenSkills: ["Carbon Offset Verification", "Forest Carbon Projects", "Verra VCS"], certifications: ["SAF Certified Forester"], yearsExperience: 7, aiSummary: "Forest ecologist with specialization in carbon offset project development and verification. Published researcher.", targetSectors: ["forestry", "conservation"] },
  });
  const finnSeeker = await prisma.seekerProfile.create({
    data: { accountId: finnAccount.id, headline: "Manufacturing Engineer | Clean Production", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "exploring-options"], skills: ["Lean Manufacturing", "Six Sigma", "Process Optimization", "CAD/CAM"], greenSkills: ["Clean Manufacturing", "Waste Reduction", "Energy Efficiency"], certifications: ["Six Sigma Green Belt", "Lean Certified"], yearsExperience: 6, aiSummary: "Manufacturing engineer with strong continuous improvement background looking to apply lean skills to sustainable production.", targetSectors: ["manufacturing", "circular-economy"], isMentor: true, mentorBio: "Lean manufacturing veteran happy to mentor people interested in sustainable manufacturing careers.", mentorTopics: ["Sustainable Manufacturing", "Lean/Six Sigma in Green Jobs", "Manufacturing Careers"], mentorSpecialty: "Sustainable Manufacturing", mentorBadge: "top_mentor", mentorRating: 4.9, mentorReviewCount: 7 },
  });
  const graceSeeker = await prisma.seekerProfile.create({
    data: { accountId: graceAccount.id, headline: "Renewable Energy Attorney | Clean Energy Law", careerStage: CareerStage.SENIOR, motivations: ["finding-job", "career-advancement"], skills: ["Energy Law", "Regulatory Compliance", "Contract Negotiation", "PPA Structuring"], greenSkills: ["Renewable Energy Project Development", "Clean Energy Policy", "Carbon Markets"], certifications: ["Bar Admission (CA, NY)"], yearsExperience: 10, aiSummary: "Clean energy attorney with decade of experience structuring PPAs and navigating renewable energy regulations across multiple states.", targetSectors: ["clean-energy", "policy"] },
  });
  const hassanSeeker = await prisma.seekerProfile.create({
    data: { accountId: hassanAccount.id, headline: "Automotive Engineer | EV Powertrain", careerStage: CareerStage.CAREER_CHANGER, motivations: ["finding-job", "learning-skills", "exploring-options"], skills: ["Powertrain Engineering", "MATLAB/Simulink", "Vehicle Dynamics", "Testing & Validation"], greenSkills: ["EV Motor Design", "Battery Management Systems"], certifications: ["FE Exam Passed"], yearsExperience: 7, aiSummary: "Experienced automotive engineer transitioning from ICE to EV powertrain design. Strong modeling and simulation background.", targetSectors: ["transportation", "clean-energy"] },
  });
  const irisSeeker = await prisma.seekerProfile.create({
    data: { accountId: irisAccount.id, headline: "Sustainable Tourism Consultant | Eco-lodge Design", careerStage: CareerStage.MID_CAREER, motivations: ["finding-job", "networking"], skills: ["Tourism Management", "Sustainable Hospitality", "Business Planning", "Marketing"], greenSkills: ["Eco-certification Standards", "Sustainable Tourism Planning", "Community Tourism"], certifications: ["Sustainable Tourism Certification (GSTC)"], yearsExperience: 5, aiSummary: "Tourism professional focused on sustainable hospitality design. Has consulted on 10+ eco-lodge and sustainable tourism projects.", targetSectors: ["tourism", "consulting"] },
  });
  const jamesSeeker = await prisma.seekerProfile.create({
    data: { accountId: jamesAccount.id, headline: "Forestry Technician | Trail Builder | Veteran", careerStage: CareerStage.RETURNING, motivations: ["finding-job", "learning-skills", "networking"], skills: ["Forestry Operations", "Trail Construction", "Chainsaw Operation", "Heavy Equipment"], greenSkills: ["Forest Management", "Trail Restoration", "Erosion Control"], certifications: ["S-212 Wildfire Chainsaw", "CDL Class B"], yearsExperience: 4, aiSummary: "Military veteran returning to civilian workforce with forestry and land management skills. Strong work ethic and leadership.", targetSectors: ["forestry", "conservation"] },
  });
  const keikoSeeker = await prisma.seekerProfile.create({
    data: { accountId: keikoAccount.id, headline: "Climate Art Director | Environmental Media", careerStage: CareerStage.SENIOR, motivations: ["exploring-options", "networking"], skills: ["Art Direction", "Brand Strategy", "Video Production", "Adobe Creative Suite"], greenSkills: ["Climate Communication", "Environmental Documentary", "Impact Storytelling"], certifications: [], yearsExperience: 10, aiSummary: "Award-winning art director and visual storyteller focused on climate and environmental narratives. Strong documentary background.", targetSectors: ["media", "arts-culture"] },
  });
  ...
  const wasteReductionJob = await prisma.job.create({
    data: {
      ...
      salaryCurrency: "USD",
      climateCategory: "Circular Economy",
      impactDescription: "Help divert 10,000+ tons of waste from landfills annually through data-driven interventions.",
      requiredCerts: [],
      greenSkills: ["Waste Auditing", "Lifecycle Assessment", "Supply Chain Sustainability", "Data Analysis"],
      status: JobStatus.PUBLISHED,
  ...
  const materialsEngineerJob = await prisma.job.create({
    data: {
      ...
      salaryCurrency: "USD",
      climateCategory: "Circular Economy",
      impactDescription: "Eliminate 500+ tons of single-use plastic per year through material innovation.",
      requiredCerts: [],
      greenSkills: ["Materials Science", "Lifecycle Assessment", "Bio-based Materials", "Polymer Chemistry"],
      status: JobStatus.PUBLISHED,
  ...
  const batteryEngineerJob = await prisma.job.create({
    data: {
      ...
      impactDescription: "Enable 2GW of renewable energy storage capacity, eliminating millions of tons of CO2 from fossil peaker plants.",
      requiredCerts: [],
      greenSkills: ["Battery Technology", "Energy Storage", "Electrochemistry", "Thermal Management"],
      status: JobStatus.PUBLISHED,
      publishedAt: new Date("2025-01-20"),
  ...
  const gridAnalystJob = await prisma.job.create({
    data: {
      ...
      salaryCurrency: "USD",
      climateCategory: "Energy Storage",
      impactDescription: "Accelerate grid decarbonization by optimizing battery storage placement and dispatch.",
      requiredCerts: [],
  ...
  const conservationTechJob = await prisma.job.create({
    data: {
      ...
      impactDescription: "Monitor and protect over 2 million acres of critical habitat using cutting-edge technology.",
  ...
  const ecosystemDataJob = await prisma.job.create({
    data: {
      ...
      salaryCurrency: "USD",
      climateCategory: "Conservation Tech",
      impactDescription: "Our AI models have identified 15 previously unknown endangered species populations.",
      requiredCerts: [],
      greenSkills: ["Machine Learning", "Ecological Modeling", "Remote Sensing", "Python", "Biodiversity Assessment"],
      status: JobStatus.PUBLISHED,
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 84,
      matchReasons: JSON.stringify({
        strengths: ["Marine biology background", "Water conservation experience", "Field research skills"],
        gaps: ["No PE License", "Limited engineering design experience"],
        overallAssessment: "Strong environmental science background. Would benefit from mentorship on engineering aspects.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  const app6 = await prisma.application.create({
    data: {
      ...
      matchScore: 90,
      matchReasons: JSON.stringify({
        strengths: ["10 years software engineering", "Full-stack TypeScript/React", "Cloud architecture experience"],
        gaps: ["No direct climate tech experience", "Career changer from fintech"],
        overallAssessment: "Excellent technical match. Strong motivation to transition into climate tech.",
      }),
      source: "LinkedIn",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 82,
      matchReasons: JSON.stringify({
        strengths: ["MBA with sustainability focus", "ESG reporting knowledge", "Financial analysis skills"],
  ...
  await prisma.application.create({
    data: {
      ...
      matchReasons: JSON.stringify({
        ...
        overallAssessment: "Motivated career changer with relevant outdoor skills. Good entry-level candidate.",
      }),
      source: "Referral",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 88,
      matchReasons: JSON.stringify({
        strengths: ["PhD in ecology", "Machine learning experience", "Published biodiversity research"],
        gaps: ["Limited industry experience", "Academic background only"],
        overallAssessment: "Outstanding academic credentials. Would be a strong hire for ML-driven conservation.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 86,
      matchReasons: JSON.stringify({
        strengths: ["Environmental law background", "Policy research experience", "Strong writing skills"],
        gaps: ["Limited quantitative analysis", "No legislative experience"],
        overallAssessment: "Excellent policy foundation with strong legal background. High potential.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 79,
      matchReasons: JSON.stringify({
        strengths: ["Electrical engineering degree", "Power systems knowledge", "5 years energy sector experience"],
        gaps: ["Limited battery-specific experience", "No electrochemistry background"],
        overallAssessment: "Solid engineering foundation. Could transition well with focused training on battery systems.",
      }),
      source: "LinkedIn",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 91,
      matchReasons: JSON.stringify({
        strengths: ["FAA Part 107 certified", "GIS expertise", "Remote sensing experience", "Conservation biology degree"],
        gaps: ["Limited AI/ML experience"],
        overallAssessment: "Near-perfect match. Has both the technical drone skills and conservation background.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 76,
      matchReasons: JSON.stringify({
        strengths: ["Power systems coursework", "Strong quantitative skills", "Python and MATLAB proficiency"],
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 85,
      matchReasons: JSON.stringify({
        strengths: ["Supply chain management experience", "Data analysis skills", "Sustainability certification"],
        gaps: ["No waste auditing experience specifically"],
        overallAssessment: "Strong analytical background with relevant sustainability experience. Good cultural fit.",
      }),
      source: "LinkedIn",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 68,
      matchReasons: JSON.stringify({
        strengths: ["Chemical engineering degree", "Lab research experience", "Passion for sustainability"],
        gaps: ["Limited materials science focus", "No bio-based materials experience", "Only 2 years experience"],
        overallAssessment: "Entry-level candidate with potential. Would need significant mentoring for this senior role.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 93,
      matchReasons: JSON.stringify({
        strengths: ["PhD in atmospheric science", "Published ML research", "Python/R expertise", "Climate modeling experience"],
        gaps: ["Limited industry experience"],
        overallAssessment: "Outstanding candidate. Academic research directly applicable to role requirements.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 77,
      matchReasons: JSON.stringify({
        strengths: ["Organic farming certification", "3 years regenerative ag experience", "Soil health knowledge"],
        gaps: ["No management experience", "Smaller scale operations only"],
        overallAssessment: "Good hands-on experience. Could grow into the management aspects with support.",
      }),
      source: "Referral",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchReasons: JSON.stringify({
        ...
        overallAssessment: "Strong technical candidate. Growing into leadership. Good mid-level fit.",
      }),
      source: "Green Jobs Board",
    },
  });
  ...
  await prisma.application.create({
    data: {
      ...
      matchScore: 83,
      matchReasons: JSON.stringify({
        strengths: ["Data science background", "Sustainability reporting experience", "Strong Python/SQL skills"],
        gaps: ["No formal ESG certification", "Limited GHG Protocol exposure"],
        overallAssessment: "Strong analytical skills with growing ESG knowledge. High-potential candidate.",
      }),
      source: "LinkedIn",
    },
  });
  ...
  await prisma.mentorAssignment.create({
    data: {
      ...
      status: MentorshipStatus.ACTIVE,
      notes: "Fatima is guiding Aisha on transitioning from marine biology to water policy and sustainable finance.",
    },
  });
  ...
  await prisma.mentorAssignment.create({
    data: {
      ...
      status: MentorshipStatus.ACTIVE,
      notes: "Marcus is sharing his electrical engineering and energy sector experience with David.",
    },
  });
  ...
  await prisma.mentorAssignment.create({
    data: {
      ...
      status: MentorshipStatus.ACTIVE,
      notes: "Wendy is mentoring Gabriel on how to break into sustainability from a non-traditional background.",
    },
  });
  ...
  await prisma.mentorAssignment.create({
    data: {
      ...
      status: MentorshipStatus.ACTIVE,
      notes: "Brian is helping Sanjay prepare for his first role in the energy sector after graduation.",
    },
  });
  ...
  await prisma.mentorAssignment.create({
    data: {
      ...
      status: MentorshipStatus.ACTIVE,
      notes: "Finn shares hands-on solar installation knowledge and helps Kai plan NABCEP certification path.",
    },
  });

  console.log("  Mentor Assignments: 7 (Carlos, Fatima, Jasmine, Marcus, Wendy, Brian, Finn mentoring)");
  ...
  await prisma.collectionJob.createMany({
    data: [
      ...
      { collectionId: planetWideSolutionsCollection.id, jobId: conservationTechJob.id, displayOrder: 4 },
      { collectionId: planetWideSolutionsCollection.id, jobId: ecosystemDataJob.id, displayOrder: 5 },
      ...
      { collectionId: knowledgeBuildersCollection.id, jobId: batteryEngineerJob.id, displayOrder: 4 },
  ...
  const aishaGoal = await prisma.goal.create({
    data: {
      ...
      description: "Complete the Association of State Floodplain Managers certification to strengthen my water resources credentials.",
      icon: "Certificate",
      progress: 40,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Create 2-3 open source projects demonstrating climate tech skills to strengthen job applications.",
      icon: "Code",
      progress: 65,
      status: GoalStatus.ACTIVE,
  ...
  const hannahGoal = await prisma.goal.create({
    data: {
      ...
      description: "Translate my PhD research into industry-relevant experience and build a professional network outside academia.",
      icon: "Briefcase",
      progress: 45,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.milestone.createMany({
    data: [
      { goalId: hannahGoal.id, title: "Update resume for industry format", completed: true, order: 1 },
      { goalId: hannahGoal.id, title: "Attend 3 industry networking events", completed: true, order: 2 },
      { goalId: hannahGoal.id, title: "Complete informational interviews (5/5)", completed: false, order: 3 },
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Self-study battery systems engineering to prepare for transition from general power systems to energy storage.",
      icon: "Lightning",
      progress: 55,
      status: GoalStatus.ACTIVE,
    },
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Earn LEED GA to complement my supply chain background for circular economy roles.",
      icon: "Leaf",
      progress: 80,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Complete and submit my research paper on ML applications in climate prediction to a peer-reviewed journal.",
      icon: "PencilSimple",
      progress: 70,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Build on existing NABCEP PV Installation Professional cert to earn the Design Specialist credential.",
      icon: "SunHorizon",
      progress: 35,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Finish the GRI Professional Certification Program to strengthen ESG reporting skills.",
      icon: "ChartBar",
      progress: 50,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Complete OSHA 30-hour construction safety training as the first step toward a solar installation career.",
      icon: "HardHat",
      progress: 10,
      status: GoalStatus.ACTIVE,
  ...
  await prisma.goal.create({
    data: {
      ...
      description: "Create 3 sample grid modeling projects using open-source tools to demonstrate skills to employers.",
      icon: "Lightning",
      progress: 25,
      status: GoalStatus.ACTIVE,
    },
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Conduct coastal ecosystem research and water quality monitoring. Analyze data from buoy networks and satellite imagery.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: false,
      description: "Architected microservices platform handling $2B+ in annual transactions. Led team of 6 engineers.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Advise Fortune 500 clients on ESG reporting, climate risk assessment, and sustainable finance strategies.",
      skills: ["ESG Reporting", "TCFD", "Climate Risk", "Financial Analysis", "Stakeholder Engagement"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Researching biodiversity loss patterns using machine learning on remote sensing data. Published 4 papers in Nature and Science.",
      skills: ["Machine Learning", "Python", "R", "Ecological Modeling", "Remote Sensing", "Scientific Writing"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Research environmental case law, draft policy briefs, and support litigation on clean air and water regulations.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      startDate: new Date("2020-03-01"),
      isCurrent: true,
      description: "Design and optimize electrical grid infrastructure. Lead renewable energy interconnection projects for utility-scale solar and wind.",
      skills: ["Power Systems", "Grid Design", "Renewable Integration", "SCADA", "Project Management"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Manage spatial data for conservation planning. Operate drones for habitat mapping and wildlife surveys.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      startDate: new Date("2022-01-01"),
      isCurrent: true,
      description: "Track and improve environmental metrics across global supply chain. Lead waste reduction initiatives in manufacturing partners.",
      skills: ["Supply Chain Management", "Sustainability Metrics", "Waste Reduction", "Data Analysis", "Vendor Management"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Develop machine learning models for climate prediction and extreme weather forecasting. Collaborate with NOAA on atmospheric data analysis.",
      skills: ["Machine Learning", "Climate Modeling", "Python", "TensorFlow", "Atmospheric Science"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      startDate: new Date("2021-06-01"),
      isCurrent: true,
      description: "Install commercial solar PV systems up to 1MW capacity. Lead small installation crews and ensure NEC compliance.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Analyze corporate sustainability data for ESG ratings. Build automated data pipelines and quality checks for environmental metrics.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Optimize EV manufacturing processes for sustainability. Lead initiatives to reduce production waste and energy consumption.",
      skills: ["Manufacturing Engineering", "Lean Manufacturing", "EV Technology", "Sustainability"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Implement regenerative farming practices including cover cropping, composting, and no-till methods. Monitor soil health metrics.",
      skills: ["Regenerative Agriculture", "Soil Health", "Cover Cropping", "Composting", "Organic Certification"],
    },
  });
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: true,
      description: "Research sustainable polymer alternatives to single-use plastics. Lab work focused on bio-based material characterization.",
  ...
  await prisma.workExperience.create({
    data: {
      ...
      isCurrent: false,
      description: "Modeled renewable energy integration scenarios for ERCOT grid. Built simulation tools in Python and MATLAB.",
      skills: ["Power Systems Modeling", "Python", "MATLAB", "Renewable Integration", "Data Analysis"],
    },
  });
...
Pro tip: Set up alerts so you don't miss new postings!`,
  ...
  console.log("  ├── Organizations: 5 (Solaris Energy, Aurora Climate, Verdant Systems, TerraWatt, Evergreen Tech)");
  ...
  if (supabaseAdmin) {
    ...
    console.log("     To make them loginable, add SUPABASE_SERVICE_ROLE_KEY to .env.local and re-seed.");
  }
}
